...
# 每个元素除以 30 (得到浮点数)
data_values = [x / 30 for x in raw_values]
...
# 【修改点 1】均值保留2位小数显示在图例中
ax.axhline(y=hzysmean_val, color=mean_color, linestyle='--', linewidth=2.5, label=f'Average ({hzysmean_val:.2f})', zorder=5)
...
for i, v in enumerate(data_values):
    if v == max_val or v == min_val:
        ...
        # 【修改点 2】移除之前的 /1000 和 'k' 逻辑，直接保留2位小数
        label_text = f"{v:.2f}"
...
ax.set_ylabel('Average daily FPs', fontsize=hzysfontsize)

ax.legend(loc='upper left', fontsize=18, frameon=False)

# 【修改点 3】由于数据是 0-160 量级，Y轴刻度使用整数即可，避免刻度出现多余的 ".00"
...
save_path = os.path.join(output_dir, 'fps_daily.pdf')
